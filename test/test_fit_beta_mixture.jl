...
@testset "Reconcile fit_beta_mixture on known datasets" begin
    # Create mixtures from MAP priors on known datasets
    ...
    pi_star_alpha = df_map_small.a .* df_map_small.b * nrow(df_large)
    pi_star_beta = (1 .- df_map_small.a) .* df_map_small.b * nrow(df_large)
    pi_star = [rand(Beta(alpha, beta), 1) 
        for (alpha, beta) in zip(pi_star_alpha, pi_star_beta)]
    pi_star = collect(Iterators.flatten(pi_star)) # Flatten
